...
- **Q: Why does the interview question with `const c` throw?** A: The `console.log(c)` runs **inside** the TDZ of `const c = "ayush"`. Hoisting is not "no declaration"; it's "declaration parked, value not yet set".
